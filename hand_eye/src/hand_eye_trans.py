...
class HandEyeTrans:
    def __init__(self):
        ...
        self.__eye2base_server('robot/eye2base'
                eye2base,
                self.__eye2base_transform
        )
        self.__pis2base_server('robot/pix2base'
                eye2base,
                self.__pix2base_transform
        )
        self.__eye_trans2base_server('robot/eye_trans2base'
                eye2base,
    ...
    def __get_robot_trans(self):
        ...
        self._base_tool_trans[0:3, 0:3] = tf.transformations.euler_matrix(abc[0], abc[1], abc[2], axes='sxyz')
        ...
        self._rtool_tool_trans[0:3, 0:3] = tf.transformations.euler_matrix(abc[0], abc[1], abc[2], axes='sxyz')
    ...
    def __eye2base_transform(self, req):
        ...
        res.tar_pose = np.array(result[:3]).reshape(-1)
        return True
    ...
    def __pix2base_transform(self, req):
        ...
        eye_obj_trans[:2] = np.multiply(eye_obj_trans[:2], [[self._camera_mat[0, 0], [self._camera_mat[1, 1]]])
        result = self._base_tool_trans * np.linalg.inv(self._rtool_tool_trans) * self._rtool_eye_trans * eye_obj_trans
        res = eye2baseResponse()
