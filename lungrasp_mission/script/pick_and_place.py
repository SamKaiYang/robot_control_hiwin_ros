#### ros cmd
#roslaunch lungrasp_mission pick_and_place.launch
import rospy
import sys
import time
import threading
...
#lungrasp strategy data srv 
from lungrasp_mission.srv import HighestSauce ,HighestSauceRequest
...
def lungrasp_data_client(data):
    rospy.wait_for_service('/get_highest_sauce')
    try:
        lungrasp_ = rospy.ServiceProxy('/get_highest_sauce', HighestSauce)
        resp = huddle_(data)
        return resp
...
def MotionItem(ItemNo):
    ...
    for case in switch(ItemNo):
        ...
        if case(Arm_cmd.MoveToTarget_Place):
            ...
            ## egg    ## chili   ##
            #######################
            # if label_detection == 'lemon':
            #     positon = [26.45, -10.22, 3, -180,0,0]
            #     robot_ctr.Step_AbsPTPCmd(positon)
            # elif label_detection == 'pepper':
            #     positon = [13.66 ,-10.22, 3, -180,0,0]
            #     robot_ctr.Step_AbsPTPCmd(positon)
            # elif label_detection == 'egg':
            #     positon = [1.65 ,-10.22, 3, -180,0,0]
            #     robot_ctr.Step_AbsPTPCmd(positon)
            # elif label_detection == 'ketchup':
            #     positon = [13.66 ,-17.99, 3, -180,0,0]
            #     robot_ctr.Step_AbsPTPCmd(positon)
            # elif label_detection == 'chili':
        ...
        if case(Arm_cmd.Get_Image):
            ...
            ###test 0921
            print("take obj data")
            lungrasp = lungrasp_data_client(1)
            ###test 0921
            if lungrasp.is_done == True:
                Obj_Data_Calculation(lungrasp.center_point_x,lungrasp.center_point_y,lungrasp.center_point_z)
                label_detection = lungrasp.confi
                obj_ang = lungrasp.ang
                MissionType_Flag = MissionType.Pick
                MotionStep += 1
                print("Get_Image")
                if len(lungrasp.confi) == 0: # If you don't see the object,Mission End
...
if __name__ == '__main__':
    ...
    try:
        if robot_ctr.is_connected():
            ...
            robot_ctr.Set_tool_number(13) #lungrasp tool
        ...
        if start_input == 2:
            lungrasp = lungrasp_data_client(1)
            print("confi",lungrasp.confi)
            print("center_point_x",lungrasp.center_point_x)
            print("center_point_y",lungrasp.center_point_y)
            print("center_point_z:",lungrasp.center_point_z)
            print("ang:",lungrasp.ang)
        if start_input == 3:
            positon =  [11.3440, 24.6059, 15.2749, 179.994, 10.002, -0.488]
            robot_ctr.Step_AbsPTPCmd(positon)
        if start_input == 4:
